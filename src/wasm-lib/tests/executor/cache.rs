...
async fn cache_test(
    ...
    for (index, variation) in variations.iter().enumerate() {
        ...
        let outcome = ctx.run_with_caching(program, None).await?;
...
"#;
    ...
    let first = result.first().unwrap().2.artifact_commands.len();
    let second = result.last().unwrap().2.artifact_commands.len();
